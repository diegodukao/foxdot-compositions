# https://github.com/Qirky/ten-lines-or-less/blob/master/algo-grave.py
...
d2 >> play("(X )( X)N{ xv[nX]}", drive=0.2, lpf=var([0,40],[28,4]),
           rate=PStep(P[5:8],[-1,-2],1)).sometimes("sample.offadd", 1, 0.75)
c1 >> play("#", dur=32, room=1, amp=2).spread()

d3 >> play("e", amp=var([0,1],[PRand(8,16)/2,1.5]), dur=PRand([1/2,1/4]),
           pan=var([-1,1],2))
...
p1 >> karp(dur=1/4, rate=PWhite(40), pan=PWhite(-1,1), amplify=var.switch,
           amp=1, room=0.5)
p2 >> sawbass(var([0,1,5,var([4,6],[14,2])],1), dur=PDur(3,8), cutoff=4000,
